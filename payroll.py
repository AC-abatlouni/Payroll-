...
def sum_spiffs_for_dept(spiffs_df: pd.DataFrame, tech_name: str, dept_code: str) -> float:
    """
    Sum spiffs (both positive and negative) for a specific technician and department code.
    Excludes TGL and commission entries from spiff calculations.
    """
    dept_spiffs = spiffs_df[
        (spiffs_df['Technician'] == tech_name) & 
        (spiffs_df['Memo'].apply(lambda x: extract_dept_code(str(x)) == dept_code)) &
        # Exclude TGL and commission entries
    ...
    # Convert amounts to float, handling currency formatting
    amounts = dept_spiffs['Amount'].apply(
        lambda x: float(str(x).replace('$', '').replace(',', '')) if pd.notnull(x) else 0.0
    )
    
    return amounts.sum()  # Sum both positive and negative amounts
...
def process_paystats(paystats_file: str, tech_data: pd.DataFrame, target_date: str, logger: logging.Logger) -> List[PayrollEntry]:
    ...
    try:
        ...
        combined_file = r'C:\Users\abatlouni\Downloads\combined_data1.xlsx'
            ...
            for dept_code in DEPARTMENT_CODES.keys():
                ...
                try:
                    ...
                    if total_col in row:
                        ...
                        if total_str:  # Process any non-empty string
                            try:
                                total_amount = float(total_str.replace('$', '').replace(',', ''))
                            except ValueError:
                                total_amount = 0.0

                    # Calculate adjusted amount (can be negative if spiffs exist without revenue)
                    adjusted_amount = round(total_amount - spiffs_total, 2)

                    # Always apply commission rate, even for negative amounts
...
def process_adjustments(combined_file: str, tech_data: pd.DataFrame, logger: logging.Logger) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    try:
        ...
        for _, row in adj_df.iterrows():
            try:
                ...
                # Get Invoice Id and remove decimal if it exists for TGL entries
                invoice_id = str(row.get('Invoice Id', ''))
                if invoice_id and 'tgl' in memo.lower():
                    try:
                        # Convert to float first in case it's already in decimal format, then to int to remove decimal
                        invoice_id = str(int(float(invoice_id)))
                    except ValueError:
                        # If conversion fails, keep original value
                        pass
                
                # Look up Badge ID from tech_data
                badge_id = tech_data.loc[tech_data['Name'] == tech_name, 'Payroll ID'].iloc[0] \
                    if not tech_data[tech_data['Name'] == tech_name].empty else "UNKNOWN"
                
                # Extract department code and get description
                ...
                # Determine if it's a TGL entry
                is_tgl = 'tgl' in memo.lower()
                ...
                adj_record = {
                    ...
                    'Memo': memo,
                    'Type': 'TGL' if is_tgl else 'Spiff',
                    'Invoice Id': invoice_id if is_tgl else ''
                }
        ...
        # Reorder columns
        column_order = ['Technician', 'Badge ID', 'Department', 'Memo', 'Type', 'Invoice Id', 'Amount']
        pos_df = pos_df[column_order]
        neg_df = neg_df[column_order]
        
        logger.info(f"Processed {len(pos_df)} positive (including TGL) and {len(neg_df)} negative adjustments")
...
def main():
    ...
    try:
        ...
        combined_file = os.path.join(base_path, 'combined_data1.xlsx')
        paystats_file = os.path.join(base_path, 'paystats1.xlsx')

        # Output files
        payroll_file = os.path.join(base_path, 'payroll1.xlsx')
        adj_pos_file = os.path.join(base_path, 'adjustments1.xlsx')
        adj_neg_file = os.path.join(base_path, 'adjustments_negative1.xlsx')
        ...
        # Process adjustments - fixed variable names
        pos_df, neg_df = process_adjustments(combined_file, tech_data, logger)

        # Save output files
        save_payroll_file(all_payroll_entries, payroll_file, logger)
        save_adjustments_files(pos_df, neg_df, adj_pos_file, adj_neg_file, logger)
