...
def process_calculations(base_path: str, logger: logging.Logger):
    ...
    try:
        ...
        # Get base date from user
        base_date = get_user_date()
        
        # Get excused hours
        excused_hours_dict = get_excused_hours(combined_file, base_date)
        
        # Process commission calculations
        results_df = process_commission_calculations(
            data, tech_data, combined_file, base_date, excused_hours_dict
        )
...
def process_payroll(base_path: str, base_date: datetime, logger: logging.Logger):
...
def main():
    ...
    try:
        ...
        base_date = process_calculations(base_path, logger)
        
        # Step 3: Process payroll
        logger.info("\nStarting payroll processing...")
        process_payroll(base_path, base_date, logger)
